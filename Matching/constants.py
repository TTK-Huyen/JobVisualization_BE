...
# Phần này là mở rộng thêm sau khi kiểm tra JD thực tế
# Chỉ thêm những skill còn thiếu nhưng thực sự xuất hiện trong dữ liệu
...
SKILL_KEYWORDS_EXTENDED_V2 = {
    "product_management_and_marketing": [
        "Product Management",
        "Product Strategy",
        "Brand Management",
        "Market Analysis",
    ...
    "business_analysis_and_product_ops": [
        "Business Analysis",
        "Requirements Documentation",
        "User Story Mapping",
        "Process Modeling",
...
ALL_SKILLS.extend([skill for group in SKILL_KEYWORDS_EXTENDED.values() for skill in group])
ALL_SKILLS.extend([skill for group in SKILL_KEYWORDS_EXTENDED_V2.values() for skill in group])
ALL_SKILLS.extend(HELPDESK_SKILLS)
